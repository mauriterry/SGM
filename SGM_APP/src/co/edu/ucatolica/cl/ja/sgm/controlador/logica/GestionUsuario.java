...
public class GestionUsuario {
    ...
    public void modificarUsuario(byte[] foto, String nombre, String correo, String membresia) {
        try {
            if (foto != null) {
                usuario.setFoto(foto);
            }
            ...
            usuario.setTipoMembresia(membresiaToBoolean(membresia));
    ...
    public void borrarHistorialDelUsuario() {
        List<Historial> historial = usuario.getHistorialList();
        for (int i = 0; i < historial.size(); i++) {
            GestionReproductor.eliminarHistorial(historial.get(i));
        }
        usuario=usuarioJpaController.findUsuario(usuario.getNickname());
    }
}
